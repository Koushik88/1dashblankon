...
<script>
    ...
    intuit.ipp.anywhere.setup({
        menuProxy: '',
        grantUrl: 'http://'+parser.hostname+'/Onedash/PHPSample/oauth.php?start=t' 
...
<?php
    ...
    echo '<div> Please refer to the <a target="_blank" href="http://localhost/Onedash/PHPSample/ReadMe.htm">Read Me</a> page for detailed instructions and information regarding this sample </div><br />';
       ...
       print_r($openid);
        ...
        if(!$openid->mode) {
            ...
            if(isset($_GET['connectWithIntuitOpenId']) || isset($_GET['subscribeFromAppsDotCom'])) {
                ...
                // echo "dhana". $_GET['openid.sreg.email'];
            ...
            echo '<br /><a href="javascript:void(0)" onclick="return intuit.ipp.anywhere.logout(function () { window.location.href = \'http://localhost/QuickbookSampleapp/PHPSample/index.php\'; });">Sign Out</a>';
			
			//oAuth code
			
					require_once('../QuickbookPhpSdk/config.php');  // Default V3 PHP SDK (v2.0.1) from IPP
...
window.location.href = "http://localhost/Onedash/PHPSample/SampleAppHomePage.php";
}

function Disconnect(parameter){
window.location.href = "http://localhost/Onedash/PHPSample/Disconnect.php";
}

function Reconnect(parameter){
window.location.href = "http://localhost/Onedash/PHPSample/Reconnect.php";
}
</script>
</body>
</html>
