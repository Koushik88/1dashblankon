...
class AdminAjaxController extends Controller {
    ...
    public function userCreation(Request $request) {
        if ($request->ajax()) {
            ...
            $url = $_SESSION["serverUrl"] . '/user_email';
            $postFields = array('adduser' => 'adduser', 'add_username' => $username, 'link' => $link, 'email' => $_POST['email'], 'password' => $password);
            $ch = curl_init($url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($postFields));
    ...
    public function deleteUserInfo(Request $request) {
        if ($request->ajax()) {
            ...
            $link = $_SESSION["serverUrl"];
            $url = $_SESSION["serverUrl"] . '/user_email';
            $postFields = array('deleteuser' => 'deleteuser', 'del_username' => $_POST["del_username"], 'link' => $link, 'email' => $_POST['email']);
            $ch = curl_init($url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($postFields));
