...
class SocialStream {
	...
	public function init($attr, $allAccessCredentials, $echo = true, $args = null, $ajax_feed = array(), $loadmore = array()) {
		// $GLOBALS['api'] = json_decode($allAccessCredentials, true);
		$GLOBALS['api'] = $allAccessCredentials;
		...
		$allAccessCredentials_ajax = json_encode($allAccessCredentials);
...
@media (max-width: 319px) { $bpcol[6] }";
		...
		if (!empty($feeds)) {
						...
						if ($feed_class == 'facebook') {
										...
										foreach ($data as $entry) {
											...
											if ($this->make_remove($link)) {
												...
												if (!empty($entry->likes->data) && $likes_count) {
													...
													$total_likes = $entry->likes->summary->total_count;
													$total_cmds = $entry->comments->summary->total_count;
												}
                            ...
                            <span class="sb-metadata">';
													if (@$facebook_output['comments'] && $comments_data) {
														$meta .= '
                                <span class="sb-meta">
                                    <span class="comments"><i class="sb-bico sb-comments"></i><bold>' . $total_cmds . '</bold>' . ' ' . ucfirst(ss_lang('comments')) . '</span>
                                </span>
                                ' . $comments_data;
													}
													...
													if (@$facebook_output['likes'] && $likes_data) {
														$meta .= '
                                <span class="sb-meta">
                                    <span class="likes"><i class="sb-bico sb-star"></i><bold>' . $total_likes . '</bold>' . ' ' . ucfirst(ss_lang('likes')) . '</span>
                                </span>
							...
							if (@$feed->items) {
								foreach ($feed->items as $item) {
									/*
										                                     *
										                                     *
										                                     *Manigandan, get each video statistics, viewCount, likeCount, dislikeCount, favoriteCount & commentCount
										                                     *
										                                     *
									*/

									$stat_url = "https://www.googleapis.com/youtube/v3/videos?id=" . $item->snippet->resourceId->videoId . "&part=statistics&key=" . @$GLOBALS['api']['google']['google_api_key'];
									$stat = (!$forceCrawl) ? $cache->get_data($stat_url, $stat_url) : $cache->do_curl($stat_url);
									$stat = @json_decode($stat);
									$statistics = @$stat->items[0]->statistics;
                                    ...
                                    <span class="sb-text">
                                        <span class="sb-meta">
                                            <span class="views"><i class="sb-bico sb-play-circle"></i><bold style="position: relative;top: -2px;padding-left: 2px;">' . @$statistics->viewCount . '</bold></span>
                                            <span class="likes"><i class="sb-bico sb-thumbs-up"></i><bold style="position: relative;top: -2px;padding-left: 2px;">' . @$statistics->likeCount . '</bold></span>
                                            <span class="likes"><i class="fa fa-thumbs-down" style="
                                            font-size: 14px;opacity: 0.7;padding-right: 5px;"></i>' . @$statistics->dislikeCount . '</bold></span>
                                            <span class="comments"><i class="sb-bico sb-comment"></i><bold style="position: relative;top: -2px;padding-left: 2px;">' . @$statistics->commentCount . '</bold></span>
                                        </span>
                                    </span>';
										//-------------------
											...
											if (@$description) {
												...
												if (@$attr['words']) {
													if (!$thumb) {
														$thumbarr = $this->getsrc($description);
														//$thumb    = $thumbarr['src'];
														if ($thumbarr['width'] != "" && $thumbarr['width'] != null) {
															$thumb = $thumbarr['src'];
														}
													}
													$text = $this->word_limiter($description, $link);
			...
			if (@$finals) {
				...
				if (empty($loadmore)) {
					$output_error = '<p class="sboard-nodata"><strong>1Dash:</strong> There is no feed data to display!</p>';
				}

			}
		} else {
			if (empty($loadmore)) {
				$output_error = '<p class="sboard-nodata"><strong>1Dash: </strong>There is no feed to show or you are not connected to the world wide web!</p>';
			}
		...
		if (!$ajax_feed) {
			...
			if (@$attr['loadmore']) {
				...
				$more_output .= '
                  $.ajax({
                    type: "post",
                    url: "socialstreamAjax",
                    data: {action: "sb_loadmore", attr: ' . $attr_ajax . ', nonce: lmnonce, label: "' . $label . '",allAccessCredentials: ' . $allAccessCredentials_ajax . '},
                    cache: false
                    })
		...
		if (!$ajax_feed) {
			$output .= ($forceCrawl) ? "\t<!-- End 1Dash - cache is disabled. -->\n" : "\t<!-- End 1Dash - cache is enabled - duration: " . $attr['cache'] . " minutes -->\n";
		}
	...
	public function get_feed($feed_key, $i, $key2, $feed_value, $results, $sboption, $cache, $forceCrawl = false, $sb_label = null) {
		...
		switch ($feed_key) {
		case 'facebook':
			$pageresults = 30; // the max results that is possible to fetch from facebook API - for group = 9 for page = 30 - we used 9 to support both
			...
			$afields = array('id', 'created_time', 'updated_time', 'link', 'from', 'name', 'source', 'message', 'description', 'story', 'comments.summary(true)', 'likes.summary(true)', 'picture', 'full_picture', 'object_id', 'type', 'status_type');
			// define the feed url
			if ($i == 1) {
				// Page Feed
		...
		case 'instagram':
			...
			if ($i == 1) {
				...
				*/
				// since our db got instagram user id, so we dont need to pass username and find userid from id
	 ...
	 */
	public function word_limiter($text, $url = '', $comment = false) {
	...
	public function title_limiter($str, $url = '') {
		$end_char = '...';
	...
	public function append_links($str) {
	...
	public function links_callback($matches) {
	...
	public function twitter_add_links($text) {
	...
	public function vk_get_photo($photo) {
...
function ss_friendly_date($date) {
	...
	if (strlen($post_time) > 10) {
		$post_time = substr($post_time, 0, -3);
	}
...
function social_stream($atts, $allAccessCredentials) {
	$sb = new SocialStream();
	return $sb->init($atts, $allAccessCredentials, false);
}
